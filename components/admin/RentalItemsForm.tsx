...
export default function RentalItemsForm({ items: initial }: { items: RentalRow[] }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {items.map(item => (
        <div key={item.id} className={`flex flex-wrap items-center gap-4 p-4 bg-white border rounded-xl ${item.available ? 'border-warm-200' : 'border-gray-200 opacity-50'}`}>
          <div className="flex-1 min-w-[120px]"><p className="font-medium text-warm-700 text-sm">{item.name}</p></div>
          <div className="flex items-center gap-1">
            ...
            <input type="number" value={item.price_per_day} onChange={e => update(item.id, 'price_per_day', Number(e.target.value))}
                   className="w-20 border border-warm-200 rounded-lg px-3 py-2 text-sm text-warm-700 focus:outline-none focus:border-warm-400" />
      ...
      <div className="p-4 bg-warm-50 border border-dashed border-warm-300 rounded-xl">
        ...
        <div className="flex flex-wrap gap-3 items-end">
          <div><label className="block text-xs text-warm-400 mb-1">品名</label><input value={newName} onChange={e => setNewName(e.target.value)} placeholder="例：ランタン" className="border border-warm-200 rounded-lg px-3 py-2 text-sm text-warm-700 focus:outline-none focus:border-warm-400 w-36" /></div>
          <div><label className="block text-xs text-warm-400 mb-1">1泊料金（円）</label><input type="number" value={newPrice} onChange={e => setNewPrice(e.target.value)} placeholder="500" className="border border-warm-200 rounded-lg px-3 py-2 text-sm text-warm-700 focus:outline-none focus:border-warm-400 w-24" /></div>
